...
void BindNodeVisitor::Visit(common::ManagedPointer<parser::InsertStatement> node) {
  ...
  auto insert_values = is_insert_select ? common::ManagedPointer(&ins_columns) : node->GetValues();
  ...
  if (is_insert_select) {
    // insert_values contains what's projected upwards so rewrite
    // the output of the select statement.
    node->GetSelect()->SetSelectColumns((*insert_values)[0]);
  }
